...
- **take**: reconciled — the nonce-correlated late reply is consumed as the
  ...
  them; never guessed). A late `pay-bond-invoice` is the exception: the
  rebuild excludes it on purpose (its amount is the bond, not the order) and
  the daemon's idempotent re-send owns that recovery.

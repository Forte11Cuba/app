...
- **FR-036**: The chat MUST support text messages, encrypted image attachments, and encrypted file attachments, **interoperable with v1** in both directions (#589):
  ...
  - **Viewing (UI)**: a received image is downloaded and decrypted on arrival, as in v1, and drawn at the shape the sender declared; tapping it opens a full-screen viewer with pinch-zoom and Save. Any other file shows a card (name, size, type) and is downloaded only when the user taps Save. Save goes through the system save dialog — the only path that writes decrypted content to disk, and only where the user chooses. "Open with…" and share are not offered yet.
